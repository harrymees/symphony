defmodule SymphonyElixir.AppServerTest do
  ...
  test "app server ignores decoded non-object JSON while waiting for responses" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "symphony-elixir-app-server-json-noise-#{System.unique_integer([:positive])}"
      )
    ...
    try do
      ...
      issue = %Issue{
        id: "issue-json-noise",
        identifier: "MT-94",
        title: "Ignore JSON scalar noise",
        description: "Ensure decoded non-object side output does not crash response handling",
        state: "In Progress",
      ...
      assert {:ok, _result} =
               AppServer.run(workspace, "Ignore JSON scalar noise", issue, on_message: on_message)
