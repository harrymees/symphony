...
hooks:
  after_create: |
    ...
    if command -v mise >/dev/null 2>&1 && [ -f mise.toml ]; then
      mise exec -- corepack pnpm install --ignore-scripts
    else
...
## Repository-specific guidance for Jam
...
- Programs do not call each other directly; they make claims and react to other claims.
- The repo uses custom JSX (`h` / `Fragment`), not React.

Critical repo guidance:
- Read `AGENTS.md` first and follow it.
- Use the repo's actual APIs and patterns: `claim`, `remember`, `replace`, `forget`, `when`, `whenever`, `transaction`, `$`, `_`.
- Keep changes aligned with the repo's malleable-software model and fact-database architecture.
- Prefer end-to-end sane fixes over compatibility shims or one-off hacks.
