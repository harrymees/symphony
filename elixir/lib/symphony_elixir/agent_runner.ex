defmodule SymphonyElixir.AgentRunner do
  ...
  require Logger
  alias SymphonyElixir.Agent.{Dispatcher, Selection}
  ...
  defp run_agent_turns(workspace, issue, codex_update_recipient, opts, worker_host) do
    ...
    with {:ok, agent_selection} <- agent_selection(issue, opts),
         {:ok, session} <- Dispatcher.start_session(agent_selection, workspace, worker_host: worker_host) do
      try do
  ...
  defp agent_selection(issue, opts) do
    case Keyword.get(opts, :agent_selection) do
      %{harness: _harness} = selection -> {:ok, selection}
      _ -> Selection.resolve(issue)
    end
  end
