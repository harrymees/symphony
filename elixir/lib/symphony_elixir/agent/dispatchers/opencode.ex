defmodule SymphonyElixir.Agent.Dispatchers.OpenCode do
  @moduledoc false
  ...
  def run_turn(session, prompt, issue, opts) do
    ...
    session_id = session_id("opencode", issue)
    metadata = metadata(session, session_id)
    on_message = Keyword.get(opts, :on_message, &default_on_message/1)

    emit(on_message, metadata, :session_started, %{session_id: session_id})

    command = render_command(settings.opencode.command, prompt, issue, session, settings, "--model")

    case CommandRunner.run(command, session.workspace, session.worker_host, settings.opencode.turn_timeout_ms, on_message, metadata) do
      {:ok, %{output: output} = result} ->
        details = opencode_completion_details(output, session_id)
  ...
  defp opencode_completion_details(output, fallback_session_id) do
    case Jason.decode(output) do
      {:ok, %{"session_id" => session_id} = payload} when is_binary(session_id) ->
        %{payload: payload, raw: output, session_id: session_id}
